...
namespace WebApi.Controllers
{
    ...
    public class ProfileController : ControllerBase
    {
        ...
        [HttpPut]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        public async Task<IActionResult> Put(EditProfileApiDto editPofileDto)
        {
            //Get UserId
            var userId = User.Claims?.FirstOrDefault(p => p.Type == "UserId")?.Value;

            //Map to app dto 
            var inputService = _mapper.Map<EditProfileDto>(editPofileDto);


            //Update prof
            var resultEditService = await _editProfileService.Execute(userId, inputService);
